"""Competitive Analysis Agent — Wave 1, Pro + grounding. Never fabricates funding."""
...
_PROMPT_PATH = Path(__file__).parent.parent / "prompts" / "competitive_analysis.txt"
...
class CompetitiveAnalysisAgent(PrometheusAgent[CompetitiveAnalysisResult]):
    name: ClassVar[AgentName] = AgentName.COMPETITIVE_ANALYSIS
    wave: ClassVar[Wave] = Wave.WAVE_1
    model: ClassVar[str] = settings.model_pro
    output_schema: ClassVar[type] = CompetitiveAnalysisResult
    ...
    def output_key(self) -> str:
        return "competitive_analysis_result"
